...
1. Start from an OutputThunk
2. Look at its pipeline_thunk.inputs
3. For each input that's an OutputThunk, recurse
4. For each input that's a BaseVariable, check if it has a vhash
5. If BaseVariable has no vhash but wraps an OutputThunk, recurse into that
...
- `find_unsaved_variables(output_thunk)` - Returns list of unsaved BaseVariables in the upstream chain
- `traverse_upstream(output_thunk, visitor_fn)` - Generic traversal with callback

### Step 4: Implement strict mode validation
**Status: DONE**

In `BaseVariable.save()` (variable.py lines 176-195):
- When data is an OutputThunk and lineage_mode="strict"
...
- If unsaved variable wraps an OutputThunk, includes source function info
...
- For each unsaved variable wrapping an OutputThunk:
  - Generates ephemeral ID: `"ephemeral:" + thunk.hash[:32]`
  - Extracts lineage from the inner OutputThunk

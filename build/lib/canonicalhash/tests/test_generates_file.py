...
class TestForEachIntegration:
    ...
    def test_for_each_dry_run_shows_metadata(self, db, capsys):
        """Dry run should show metadata being passed when pass_metadata is active."""
        RawSignal.save(np.array([1, 2, 3]), subject=1, session="A")
        ...
        assert "pass metadata" in captured.out


# --- pass_metadata override tests ---

class TestPassMetadataOverride:
    """Tests for explicit pass_metadata parameter."""

    def test_pass_metadata_true_forces_passing(self, db):
        """pass_metadata=True forces metadata passing for non-generates_file fn."""
        RawSignal.save(np.array([1, 2, 3]), subject=1, session="A")

        received_kwargs = {}

        @thunk
        def process(data, subject, session):
            nonlocal received_kwargs
            received_kwargs = {"subject": subject, "session": session}
            return data * 2
    ...
    def test_pass_metadata_false_suppresses_passing(self, db):
        """pass_metadata=False suppresses metadata passing for generates_file fn."""
        RawSignal.save(np.array([1, 2, 3]), subject=1, session="A")
        ...
        # pass_metadata=False should suppress auto-detection of generates_file
        for_each(
            make_plot,
            inputs={"data": RawSignal},
            outputs=[Figure],
        ...
        assert received_kwargs == {"called": True}

    def test_pass_metadata_none_defaults_to_generates_file(self, db):
        """pass_metadata=None (default) uses generates_file to decide."""
        RawSignal.save(np.array([1, 2, 3]), subject=1, session="A")
        ...
        def make_plot(data, subject, session):
            ...
            received_metadata = {"subject": subject, "session": session}
            return None
        ...
        for_each(
            ...
            inputs={"data": RawSignal},
            outputs=[Figure],
            # pass_metadata is not set → defaults to None → uses generates_file
            subject=[1],
            session=["A"],
        )

        assert received_metadata == {"subject": 1, "session": "A"}
